...
math: true
diagrams: true
mentorIntro: 这不是考试，也不会给你贴标签。它只是帮我们找到：哪一小块基础正在挡住你，下一步最值得先补什么。
mentorAnalogy: 出发前检查工具箱。不是工具越多越好，而是先确认今天要用的扳手在不在、会不会用。
mentorFirstStep: 拿一张纸做会做的题；不会的直接写“不知道”，不要查答案，也不要在一道题上停太久。
mentorStuck: 一题想了 3 分钟还没思路，就画圈跳过。诊断的价值正是把这个卡点找出来。
---
...
整页内容可以分两次完成。第一次先用 $20$–$35$ 分钟找到卡点，不计分；第二次再看解释、做最小代码并订正。单位、角度、坐标方向和数组维度属于后续课程的安全地基，但一次不会并不代表要重学整本数学。
...
- 第一次不要查资料，也不要运行代码；建议最多用 $35$ 分钟。
- 每题先标记：**会做**、**有印象但说不清**、**完全陌生**。
- 会做的写过程；完全陌生的直接写“不知道”，这就是有效答案。
- 一题卡住超过 3 分钟就跳过。完成后再看解释，用另一种颜色补上自己刚弄懂的部分。
...
<details class="mentor-reference">
  <summary>导师复测参考（第一次学习不用看）</summary>
  <div class="mentor-reference-body">
    <p>第一次只用它找起点，不计分。准备进入下一阶段时，导师才会和你一起看下面的参考。</p>
    <div class="table-wrap" role="region" aria-label="导师复测参考表，可横向滚动" tabindex="0">
      <table>
        <thead><tr><th>等级</th><th>现在能做到什么</th><th>下一步</th></tr></thead>
        <tbody>
          <tr><td>0</td><td>变量、单位、角度或 shape 还说不清</td><td>先挑一个最小例子一起做</td></tr>
          <tr><td>1</td><td>在提示下能做一步，但容易混淆单位、象限或 axis</td><td>只补对应的小节，再换数字试一次</td></tr>
          <tr><td>2</td><td>能独立完成、用代码检查并订正错误</td><td>可以进入数学核心课</td></tr>
          <tr><td>3</td><td>还能解释常见错误为什么发生，并设计检查</td><td>可以帮助同伴复核结果</td></tr>
        </tbody>
      </table>
    </div>
    <p>导师复测的参考是总分至少 \(19/24\)，且第 1、5、7、8、9 题达到等级 2。没有达到时只补相应知识，不需要重学全部内容。</p>
  </div>
</details>
