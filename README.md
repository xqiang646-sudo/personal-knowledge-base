...
- `update.html`：实习生在当前设备维护学习卡、知识补充草稿和导出备份。

第一版进度和学习卡使用浏览器 `localStorage`。学习卡可编辑、删除、复制为 Markdown，并支持 JSON 导入导出；它们不会自动上传，导师验收和实习生作业也不进入公开站点。

仓库内置 `.agents/skills/first-principles-learning/SKILL.md`。在仓库中使用 Codex 时，可以输入：

```text
使用 $first-principles-learning，先不要给答案，带我从第一性原理理解坐标系转换。
```

页面和 Skill 共用 `src/_data/learningRecord.json` 作为学习记录合同。知识补充只生成私人草稿；确认内容与隐私边界后，再由导师审阅代码变更。
...
- `.agents/skills/first-principles-learning/`：面向零基础实习生的第一性原理教学 Skill。
