...
function resolveTarget(sourceFile,href){const [rawPath,rawHash]=href.split("#",2);const hash=rawHash?decodeURIComponent(rawHash):"";let targetPath=decodeURIComponent(rawPath||"");if(targetPath.startsWith(prefix))targetPath=`/${targetPath.slice(prefix.length)}`;let absolute;if(!targetPath)absolute=sourceFile;else if(targetPath.startsWith("/"))absolute=path.join(output,targetPath.slice(1));else absolute=path.resolve(path.dirname(sourceFile),targetPath);if(absolute.endsWith(path.sep)||(fs.existsSync(absolute)&&fs.statSync(absolute).isDirectory()))absolute=path.join(absolute,"index.html");return {absolute,hash};}
