(function () {
  ...
  const LABELS = {"not-started":"还没开始","in-progress":"我正在学","artifact-ready":"我能独立做一次"};
  ...
  function render(state){
    ...
    const recorded=Object.keys(state.courses).length;
    const summary=document.querySelector("[data-progress-summary]");const bar=document.querySelector("[data-progress-bar]");
    if(summary)summary.textContent=recorded?ready+" / "+cards.length+" 门能独立做":"先选一条最适合你的起步路线";
    if(bar)bar.style.width=(cards.length?(ready/cards.length)*100:0)+"%";
    const continueLink=document.querySelector("[data-continue-link]");
    if(continueLink&&state.lastCourseUrl){continueLink.href=state.lastCourseUrl;continueLink.textContent="接着上次的位置 →";}
  }
